...
from .models import Customer, CustomerType, CustomerCategory1, CustomerCategory2, CustomerCategory3, ImpApply, TestApply
from .common import (
...
class CustomersMine(CustomerBaseView):
    ...
    def get_context_data(self, **kwargs):
        businessman = self.request.user.usercredit
        customers = businessman.customer_set.filter(is_delete=False)
        ...
        if self.request.method == 'POST':
            ...
            if self.request.POST.get("check_customer_value") == "check_customer_value" and customer_name:
...
class CustomersAll(CustomerBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'date_end': date_end,
            'export_url_param': query_string,
            'login_user':self.request.user.usercredit,
            'customers':customers,
        })
        return context
...
class CustomerList(CustomersAll):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["login_user"].role.role == '商务总监' :                                                                                                        
            # 筛选
            if context["login_user"].zone.name == '全国' :
                #print context["login_user"].zone
                return context
            else:
                #print context['customers']
                customers = context['customers'].filter(businessman__zone = context["login_user"].zone)
                #print customers
                customer_name = self.request.POST.get('customer-name','').strip()
                if self.request.POST.get("check_customer_value") == "check_customer_value" and customer_name:
                ...
                page = paginate(customers.distinct(), current_page= context['page_id'])
                context.update({
                'page': page,
                })
                return  context
...
class CustomerAdd(CustomerBaseView):
    ...
    allowed_role = [u'商务', u'商务总监']

    def get_context_data(self, **kwargs):
        context = super(CustomerAdd, self).get_context_data(**kwargs)
        context['customers'] = self.request.user.usercredit.customer_set.filter(is_delete=False)
        context['provinces'] = PROVINCES
        context['zones'] = Zone.zone_use()
        zone_businessman = self.request.user.usercredit.zone.usercredit_set.filter(role__role__in=[u'商务',u'商务总监']).all()  #zhao
        if not self.request.user.usercredit.test_flag:
...
def ajax_customer_add(request):
    ...
    try:
        ...
        # 判断客户名是否存在
        if Customer.objects.filter(name=customer_name, is_delete=False).exists():
...
def ajax_customer_edit(request):
    ...
    if not all([customer_name, customer_short_name, city, registered_capital, shareholder, product_desc]):
        return JsonResponse({'msg': u'有必填项为空'})
    try:
        customer = Customer.objects.get(id=customer_id, is_delete=False)
...
def ajax_customer_del(request):
    ...
    for one in customer_projects:
        one.is_delete = True
        one.save()
        imps = one.impapply_set.all()
        for imp in imps:
            imp.is_delete = True
            imp.save()
        tests = one.testapply_set.all()
        for test in tests:
            test.is_delete = True
            test.save()
