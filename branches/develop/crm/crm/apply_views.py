...
from api.tasks import igt_task
...
def imp_modules_config(request):
    ...
    api_pgbd_dq = JsonConfig.objects.filter(name='api_pgbd_dq').first().value
    web_pgbd_dq = JsonConfig.objects.filter(name='web_pgbd_dq').first().value
    api_pgbd_dz = JsonConfig.objects.filter(name='api_pgbd_dz').first().value
    web_pgbd_dz = JsonConfig.objects.filter(name='web_pgbd_dz').first().value
    api_hn_dq = JsonConfig.objects.filter(name='api_hn_dq').first().value
    web_hn_dq = JsonConfig.objects.filter(name='web_hn_dq').first().value
    device_serven_element = JsonConfig.objects.filter(name='device_serven_element').first().value
    return JsonResponse({'msg': 0, 'data': {'web_modules': web_modules, 
                                            'no_test': no_test_modules, 
                                            'webModules_hna_api':web_modules_hnaapi,
                                            "api_pgbd_dq":api_pgbd_dq,
                                            "web_pgbd_dq":web_pgbd_dq,
                                            "api_pgbd_dz":api_pgbd_dz,
                                            "web_pgbd_dz":web_pgbd_dz,
                                            "api_hn_dq":api_hn_dq,
                                            "web_hn_dq":web_hn_dq,
                                            "device_serven_element":device_serven_element
                                             }})
...
class ImpApplyAdd(CommonView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            deviceSenvenElement = self.get_device_anti_fraud(request)
            context = self.get_context_data(**kwargs)
            ...
            extra_fields['deviceSenvenElement'] = deviceSenvenElement
...
class ImpApplyEdit(CommonView):
    ...
    def post(self, request, *args, **kwargs):
        self.check_role()
        print self.get_device_anti_fraud(request)
...
def imp_apply_progress_add(request, imp_apply_id):
    ...
    try:
        if progress == 11:
            ...
            return_to_applicant_email([imp_apply.applicant.user.email, ],imp_apply.id ,email_type = u"退回邮件", returned_person = user_name ,turn_reason = description)
            body = "【{0}】【{1}】{2}".format(imp_apply.id,imp_apply.project.name,"交付申请工单被退回了，请处理")
            ## getui tui song xiao xi start
            igt_task.delay(title,body,cid,imp_apply)
            ## getui tui song xiao xi end
        elif progress == 13:
            ...
            ## getui tui song xiao xi start
            body = "【{0}】【{1}】{2}".format(imp_apply.id,imp_apply.project.name,"已被运营审核通过啦!!!")
            # use celery yibu zhixing grtui
            igt_task.delay(title,body,cid,imp_apply)
            ## getui tui song xiao xi end
        ########
        # added by guo 
        #########
            ...
            title = "CRM消息通知"
            body = "【{0}】【{1}】{2}".format(imp_apply.id,imp_apply.project.name,"交付完成!!!")
            igt_task.delay(title,body,cid)
...
def imp_apply_done(request):
    ...
    title = "CRM消息通知"
    body = "【{0}】【{1}】{2}".format(imp_apply.id,imp_apply.project.name,"交付工程师已经开通apicode账号了!!!")
    igt_task.delay(title,body,cid,imp_apply)
    extra_fields = imp_apply.extra_fields
