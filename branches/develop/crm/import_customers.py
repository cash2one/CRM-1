...
# book = xlrd.open_workbook('customers.xls')
...
# 
# no_customer = open('no_customers3.csv', 'wb')
...
# print '----------------------------------------------'
# 
# book1 = xlrd.open_workbook('customers1.xls')
...
#         customer_type = CustomerType.objects.filter(name=u'其他').first()
...
data = read_xls(book1, u'交付系统有的客户但crm系统没有客户')
for d in data:
    customer = Customer.objects.filter(name=d[1]).first()
    if customer:
        customer.short_name = d[2]
        customer.save()
