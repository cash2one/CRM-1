...
function table_del(self){
    ...
    //alert(lineNum)
    //alert($("#table-contact tbody tr:eq("+lineNum+")").val());
    $("#table-contact tbody tr:eq("+lineNum+")").remove();
    table_line_each();
...
function table_line_each(){
    $("#table-contact tr:gt(0)").each(function(i){
        $(this).children("th").html(i);
        $(this).find("button").attr("value", i);
    });
}
...
function ajaxContactCreate(projectId){
    ...
    var data = {
        ...
        pos: modal.find(".modal-body select[name='contact-pos']").val(),
...
function contact_add(){
    ...
    var pos = modal.find(".modal-body select[name='contact-pos']").val();
...
function show_contact_edit(self){
    ...
    modal.find(".modal-body select[name='contact-pos']").val(contact_this_info[1]);
...
function checkProjectLegal(project_name){
      if(project_name!=""){
      var rel = /[-－]+/g;//只能输入汉字和英文字母
      singal = rel.test(project_name);
      if(singal== true){
              alert("项目名字不能包含－-！");
              return false;
            }
...
function ajax_project_add(){
    ...
    //检查项目名字不能包含- －雷达的要求
    check = checkProjectLegal(project_name)
    if (check==false){
        return false;
    }
