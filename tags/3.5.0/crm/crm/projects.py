...
class Projects(ProjectListBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['projects'] = filter_project(
            self.request.user.usercredit.project_set.all(),
...
class ProjectList(ProjectListBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if role == u'商务总监':
            ...
            projects = projects.filter(customer__zone=zone)
            zoneid = zone.id
...
class ProjectListProduct(ProjectListBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        projects = Project.objects.filter(products__in=self.request.user.usercredit.product_set.all())
        # 过滤
        projects = filter_project(
...
class ProjectListAnalyser(ProjectListBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        projects = Project.objects.filter(testapply__analyser=self.request.user.usercredit)
        # 根据测试状态过滤项目
...
def ajax_project_del(request):
    ...
    try:
        ...
        return JsonResponse({'msg': unicode(e)})
    project.impapply_set.all().delete()
    project.testapply_set.all().delete()
    project.progress_set.all().delete()
