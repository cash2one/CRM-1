...
class ImpApplyDirectorListView(CommonView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['imp_engineer_list'] = UserCredit.objects.filter(Q(role__role='交付经理') | Q(role__role='交付总监'))
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            change_log = request.user.first_name + ',将项目' + impapply.project.name + ',指派给' + imp_engineer.user.first_name + ',原因是:' +context['change_reason']
        ...
        try:
            ...
            if progress == 20:
                try:
                    ...
                    send_to_impapply_email([analyser.user.email],imp.id ,email_type = u"通知")
                except Exception:
                    ERR_LOG.error(traceback.format_exc())
...
class ImpApplyEdit(CommonView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if progress == 20:
            ...
            try:
                #email_alert([analyser.user.email], alert=2)  # 邮件提醒风控经理审核
                send_to_impapply_email([analyser.user.email],imp_apply.id,email_type = u"交付申请")
            except Exception as e:
                ERR_LOG.error(traceback.format_exc())
...
def imp_apply_view(request, imp_apply_id):
    deliver_status = request.POST.get("deliver_status",0)
    deliver_description = request.POST.get("description",0)
...
def imp_apply_progress_add(request, imp_apply_id):
    ...
    try:
        ...
        return JsonResponse({'msg': unicode(e)})
    description = request.GET.get('description', u'qweqwweq')
    ...
    try:
        if progress == 11:
            ...
            return_to_applicant_email([imp_apply.applicant.user.email, imp_apply.analyser.user.email],imp_apply.id ,email_type = u"退回邮件", returned_person = user_name ,turn_reason = description)
        elif progress == 13:
