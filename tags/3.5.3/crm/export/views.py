        ...
        for imp_apply in impapply:
            ...
            if extra_fields.get("pgbg_product") and extra_fields.get("pgbg_product").get("product_selected").values():
                for p in extra_fields.get("pgbg_product").get("product_selected").values():
                     report_string = report_string + p.get("name") + '（'+ p.get("code") +'）'+'；' +'\n' 
                     if p.get("price"):
                        report_string = report_string[:-1]
                        report_string = report_string +'单价（元）'+'：'+ p.get("price") +'\n' 
            imp_HNApi = ''
            if extra_fields.get("HNApi_selected") and extra_fields.get("HNApi_selected").values():
                for p in extra_fields.get("HNApi_selected").values():
                    if extra_fields.get("account_type") == "1":
                        imp_HNApi =imp_HNApi + p.get("name") +  '；' + '测试条数：' + p.get("val") + '说明：' +  p.get("note") +"\n"
                    else:
                        imp_HNApi =imp_HNApi + p.get("name") +  '；' + '单价：' + p.get("val") + '说明：' +  p.get("note") + "\n"
            update_record = ''
            if imp_apply.impapplyprogress_set.all():
                for p in imp_apply.impapplyprogress_set.all() :
                       update_record = update_record+p.create_time.strftime("%Y-%m-%d %H:%M:%S") + p.operator + p.description +  progress_data[int(p.progress)] + "\n"
            try:
                extra_fields = imp_apply.extra_fields
                if extra_fields:
            ...
            row = [ 
                ...
                pay_type.get(cm.get('mode')),cm.get("hit_price"),cm.get("fxgd_note"),cm.get("year_price",'none')+ "元/" +cm.get("month_price",'none')+"月",
                report_string , imp_HNApi ,cus.get("cus_pf"),cus.get("cus_data"),
                cus.get("rule_blk_sheet"),cus.get("rule_multi_apply"),extra_fields.get("radar_permission"),
                extra_fields.get("notes"),
                imp_apply.contact.name, imp_apply.contact.position, '`' + imp_apply.contact.tel, imp_apply.contact.mobile,
...
def export_impapply_to_csv(request,**kwargs):
    ...
    header = [
        ...
        u"此次交付申请简况", u"补充说明", u"对接形式", u"转正后是否计入多次申请",
        ...
        u"交付的评估报告系列产品",u"交付的海纳api产品" ,u"客制化评分",u"客制化数据",
